/**
 * Browser-compatible PCRE2 wrapper
 * Fork of @stephen-riley/pcre2-wasm with Node.js dependencies removed
 *
 * Changes from original:
 * - Converted to ESM module format
 * - Replaced require("assert") with inline assertion
 * - Replaced require("util").TextDecoder with browser-native TextDecoder
 * - Replaced Buffer.from(str, 'utf16le') with custom encodeUTF16LE function
 */
...
// Browser-native TextDecoder for UTF-16LE
const utf16Decoder = new TextDecoder('utf-16le');

// Encode string to UTF-16LE (replacement for Buffer.from(str, 'utf16le'))
...
class PCRE {
  ...
  match(subject, start) {
    ...
    const subjectBuffer = encodeUTF16LE(subject);
  ...
  substitute(subject, replacement, startOffset, options) {
    ...
    const subjectBuffer = encodeUTF16LE(subject);
    const replacementBuffer = encodeUTF16LE(replacement);
    let factor = 1.5;
